...
class MohidLand:
    ...
    def process_models(self, days_run: int) :
        main_path = Path(self.yaml['MOHID_LAND']['MAIN_PATH'])
        days_per_run = self.yaml['SIMULATION']['DAYS_PER_RUN']
        self.check_triggers(days_run, self.yaml['SIMULATION']['DAYS_PER_RUN'])
        MOHID_LAND_config = self.yaml['MOHID_LAND']
        MOHID_LAND_models = MOHID_LAND_config['MODELS']
        for model in MOHID_LAND_models.keys():
            folder_utils.create_model_folder_structure(self.yaml, MOHID_LAND_models[model])
            self.change_model_dat(MOHID_LAND_models[model], main_path)
            self.gather_boundary_conditions(MOHID_LAND_models[model], main_path)
            self.gather_restart_files(MOHID_LAND_models[model], main_path)

            #TODO fix meteo
            if 'METEO' in MOHID_LAND_models[model].keys():
                for meteo_model in MOHID_LAND_models[model]['METEO']['MODELS'].keys():
                    if 'ENABLE' in MOHID_LAND_models[model]['METEO']['MODELS'][meteo_model].keys() \
                            and MOHID_LAND_models[model]['METEO'][meteo_model]['MODELS']['ENABLE']:
                        self.get_meteo_file(MOHID_LAND_models[model]['METEO']['MODELS'][meteo_model])

            if 'DISCHARGES' in MOHID_LAND_models[model].keys():
                for discharge in MOHID_LAND_models[model]['DISCHARGES'].keys():
                    if 'ENABLE' in MOHID_LAND_models[model]['DISCHARGES'][discharge].keys() \
                            and MOHID_LAND_models[model]['DISCHARGES'][discharge]['ENABLE']:
                        self.gather_discharges_files(MOHID_LAND_models[model])

        self.write_trigger(self.yaml['MOHID_LAND']['MAIN_PATH'],
    ...
    def run_mohid(self):
        output_file_name = Path(self.yaml['MOHID_LAND']['TREE_PATH'] + "MOHID_RUN_" + self.current_initial_date.strftime("%Y-%m-%d") + ".log")
        output_file = open(output_file_name, "w+")
        exe_path = Path(self.yaml['MOHID_LAND']['EXE_PATH'])
        tree_path = Path(self.yaml['MOHID_LAND']['TREE_PATH'])

        if 'MPI' in self.yaml['MOHID_LAND'].keys() and self.yaml['MOHID_LAND']['MPI']['ENABLE']:
            ...
            if 'EXE_PATH' in self.yaml['MOHID_LAND']['MPI']:
                mpi_exe_path = Path(self.yaml['MOHID_LAND']['MPI']['EXE_PATH'])
            else:
                mpi_exe_path = "mpiexec"
                self.logger.info(
                    "Executable information for MPI missing. Defaulting to main EXE: " + exe_path.__str__())
            # subprocess.run([mpi_exe_path.__str__(), "-np", str(self.yaml['MOHID_LAND']['MPI']['TOTAL_PROCESSORS']),
                            # str(exe_path), "&"], cwd=os.path.dirname(self.yaml['MOHID_LAND']['EXE_PATH']),
                           # stdout=output_file)
            subprocess.run([mpi_exe_path.__str__(), "-np", str(self.yaml['MOHID_LAND']['MPI']['TOTAL_PROCESSORS']),
                            str(exe_path), "&"], cwd=os.path.dirname(self.yaml['MOHID_LAND']['TREE_PATH']),
                           stdout=output_file)
            output_file.close()
            ...
            ddx_output_folder= Path(self.yaml['MOHID_LAND']['TREE_PATH'] + r"/DomainDecomposition_Logs/")
            
            if not os.path.isdir(ddx_output_folder):
                os.makedirs(ddx_output_folder)
                
            ddc_output_filename = Path(self.yaml['MOHID_LAND']['TREE_PATH'] + "/DomainDecomposition_Logs/" + "DomainDecomposition_" + self.current_initial_date.strftime("%Y-%m-%d") + ".log")
            mohid_ddc_output_log = open(ddc_output_filename, "w+")
            # subprocess.run(["MohidDDC.exe", str(exe_path), "&"], cwd=os.path.dirname(self.yaml['MOHID_LAND']['EXE_PATH']),
                           # stdout=mohid_ddc_output_log)
            mohid_ddc_path = Path(self.yaml['MOHID_LAND']['TREE_PATH'] + "DomainDecompositionConsolidation.exe")
            subprocess.run([mohid_ddc_path.__str__(), str(exe_path), "&"], cwd=os.path.dirname(self.yaml['MOHID_LAND']['TREE_PATH']),
                           stdout=mohid_ddc_output_log)
    ...
    def change_model_dat(self, model, main_path: Path):
        ...
        if 'OPENMP' in self.yaml['MOHID_LAND'].keys() and self.yaml['MOHID_LAND']['OPENMP']['ENABLE']:
            if 'TOTAL_PROCESSORS' in self.yaml['MOHID_LAND']['OPENMP']:
                num_omp_processors = self.yaml['MOHID_LAND']['OPENMP']['TOTAL_PROCESSORS']
            else:
    ...
    def gather_restart_files(self, model: dict, main_path:Path):
        ...
        date_format = "%Y-%m-%d"
        if 'dateFormat' in self.yaml['MOHID_LAND'].keys():
            date_format = self.yaml['MOHID_LAND']['DATE_FORMAT']
    ...
    def gather_discharges_files(self, model):
        ...
        for discharge in model['DISCHARGES']:
            ...
            files = glob.glob(path_discharges_files.__str__() + r"*.*")
    ...
    def backup_simulation(self):
        ...
        date_format = "%Y-%m-%d"
        if 'DATE_FORMAT' in self.yaml['MOHID_LAND'].keys():
            date_format = self.yaml['MOHID_LAND']['DATE_FORMAT']
       ...
       # for model in self.yaml.keys():
        for model in self.yaml['MOHID_LAND']['MODELS'].keys():
            model_keys = self.yaml['MOHID_LAND']['MODELS'][model].keys()
            results_path_end = Path(self.yaml['MOHID_LAND']['MODELS'][model]["PATH"] + "res/")
            results_path = main_path / results_path_end

            generic_path = Path(self.yaml['MOHID_LAND']['MODELS'][model]['STORAGE_PATH'])
            ...
            if 'HAS_SOLUTION_FROM_FILE' not in model_keys or not self.yaml['MOHID_LAND']['MODELS'][model]['HAS_SOLUTION_FROM_FILE']:
            ...
            hdf5_files = glob.glob(results_path.__str__() + r"/*.hdf5")
            if len(hdf5_files) > 0:
                ...
                if 'RESULTS_LIST' in model_keys:
                    for file in hdf5_files:
                        ...
                        if file_name not in self.yaml['MOHID_LAND']['MODELS'][model]['RESULTS_LIST']:
                            continue
