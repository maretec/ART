artconfig_modules = ['MAIN_PATH', 'OPERATIONAL_MODE', 'RUN_PRE_PROCESSING', 'DAYS_PER_RUN', 'REF_DAYS_TO_START',
                     'NUMBER_OF_RUNS', 'MODULE', 'RUN_SIMULATION_MODULE', 'RUN_SIMULATION_POST', 'START_DATE',
                     'END_DATE', 'OUTPUT_TO_FILE', 'OUTPUT_FILEPATH', 'SEND_EMAIL', 'EMAIL']
...
discharges_modules = ['#discharge block name']
discharge_block_modules = ['ENABLE', 'PATH', 'DATE_FORMAT']
obc_modules = ['#obc block name']
obc_block_modules = ['ENABLE', 'FILE_TYPE', 'SIMULATED_DAYS', 'SUB_FOLDERS', 'DATE_IN_FILENAME', 'DATE_FORMAT', 'FILES',
                     'WORKPATH']
meteo_modules = ['ENABLE', 'MODELS']
models_modules = ['#put meteo model Id here']
uniqueId_modules = ['NAME', 'SIMULATED_DAYS', 'FILENAME_FROM_MODEL', 'WORKPATH', 'DATE_FORMAT', 'FILE_TYPE']
modeldat_modules = ['MAXDT', 'GMTREFERENCE', 'DT_PREDICTION_INTERVAL']
preprocessing_modules = ['#name of block']
name_of_block_modules_pre = ['RUN', 'DAT_DATE_CHANGE', 'WORKING_DIRECTORY', 'CONFIG_FILEPATH', 'EXE_PATH', 'FLAGS',
                             'OUTPUT_TO_FILE', 'OUTPUT_FILEPATH']
postprocessing_modules = ['#name of block']
name_of_block_modules_post = ['RUN', 'DAT_DATE_CHANGE', 'WORKING_DIRECTORY', 'CONFIG_FILEPATH', 'EXE_PATH', 'FLAGS',
                              'OUTPUT_TO_FILE', 'OUTPUT_FILEPATH']
...
def create_discharges_block(filename):
    ...
    for i in discharges_modules:
        filename.write("    " + i + ":\n")
        for i in discharge_block_modules:
            filename.write("      " + i + ":\n")
...
def create_obc_block(filename):
    ...
    for i in obc_modules:
        filename.write("    " + i + ":\n")
        for i in obc_block_modules:
            if i == "files":
                filename.write(
                    "      " + i + ": [<write list of files you want from the OBC workpath (e.g. ['Hydrodynamic', 'WaterProperties']). Different files require a new list>]\n")
            filename.write("      " + i + ":\n")
...
def create_yaml():
    ...
    for i in range(number_model_blocks):
        ...
        create_obc_block(f)
        create_meteo_block(f, number_model_blocks[i])
